...
spID <- read.csv("Top1.csv")
crAssignment <- read.csv("Indiv_Assignments_Columbia.csv")
northAssignement <- read.csv("Indiv_Assignments_North.csv")
southAssignement <- read.csv("Indiv_Assignments_South.csv")
snppit_Parentage <- read_delim("snppit_output_ParentageAssignments.txt", delim = "\t", escape_double = FALSE, trim_ws = TRUE)
genos <- read_excel("JSOES_2024_May_Genotypes.xlsx", sheet = "Ots334")
```
...
CR_merge[, c("Genetic_Stock_Best_Estimate",
             ...
             "Probability_2nd_Best_Estimate")] <- t(apply(CR_merge[, c("Deschutes_R_fa", "Mid.Up_Columbia_R_sp", "Snake_R_fa","Snake_R_sp","Spring_Cr_Group_tule", "Up_Columbia_R_su.fa", "W_Cascade_fa", "W_Cascade_sp")], 1, function(x) 
  {
