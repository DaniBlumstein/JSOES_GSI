...
mixture <- read_excel("input_files/JSOES_2024_May_Genotypes.xlsx", sheet = "SpID176")
...
## Write table
write.csv(Top2, file = "output/speciesID_Top2.csv")
write.csv(Top1, file = "output/speciesID_Top1.csv")
...
write.csv(subset(map_rows_df, select = c("mixture_collection", "indiv", "repunit", "collection", "PofZ", "log_likelihood", "z_score", "n_non_miss_loci")), 
          file = 'output/speciesID_z-scores.csv', row.names = FALSE)

unique(rep_mix_est$repunit)
```

all things below here are not needed. but keeping here just incase
